...
import com.nbclass.common.constant.ScheduleConstants;
import com.nbclass.common.exception.TaskException;
import com.nbclass.common.util.StringUtils;
import com.nbclass.common.util.text.Convert;
import com.nbclass.modules.job.mapper.SysJobMapper;
import com.nbclass.modules.job.model.SysJob;
import com.nbclass.modules.job.service.SysJobService;
import com.nbclass.modules.job.util.CronUtils;
import com.nbclass.modules.job.util.ScheduleUtils;
...
public class SysJobServiceImpl implements SysJobService {
	...
	public int updateJob(SysJob job) throws SchedulerException, TaskException {
		SysJob properties = sysJobMapper.selectByPrimaryKey(job);
	...
	public void deleteJobByIds(String ids) throws SchedulerException {
		...
		for (Long jobId : jobIds) {
			...
			SysJob job = sysJobMapper.selectByPrimaryKey(sysjob);
			deleteJob(job);
		}
	}
	...
	public int deleteJob(SysJob job) throws SchedulerException {
		...
		int rows = sysJobMapper.delete(job);
		if (rows > 0) {
	...
	public List<SysJob> listSysJobs(SysJob sysJob) {
		...
		if (StringUtils.isNotEmpty(sysJob.getJobName())) {
			criteria.andLike("jobName", "%" + sysJob.getJobName() + "%");
		}
	...
	public SysJob getSysJobById(Long jobId) {
		SysJob sysJob = new SysJob();
		sysJob.setJobId(jobId);
		return sysJobMapper.selectByPrimaryKey(sysJob);
	}

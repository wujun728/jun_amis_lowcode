    	...
    	table: {
            ...
            responseHandler: function(res) {
				if (res.code == 200) {
					...
					//401 未授权处理 //跳转到登录页面
					$.modal.alertWarning(res.msg+"3秒后退出重新登录!");
					setTimeout(function(){
    		...
    		// 导出下载
			exportExcelByFromData: function(formId) {
				var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
				var params = $("#bootstrap-table").bootstrapTable('getOptions');
				params.queryParams = function(params) {
					var search = {};
				...
				this.exportDownload($.table._option.exportExcel,params,'testexportfilename');
    		},
			//导出下载原生ajax方法
			exportDownload: function (url, params, filename) {
				//原生ajax
				...
				//返回成功，导出的Excel文件
				xhr.onload = function (resule) {
					if (resule.currentTarget.status == 200) {
						var blob = resule.currentTarget.response;
						var a = document.createElement('a');
						var url = window.URL.createObjectURL(blob);
						a.href = url;
						//设置文件名称
						a.download = '设备报修列表导出.xlsx';
						a.click();
					}
				}

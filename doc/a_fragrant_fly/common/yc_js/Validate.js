...
export default {
    ...
    checkName: function(name, min, max) {
        var min = min || 8;
        var max = max || 12;
        var reg = "/\^[\\w]{" + min + "," + max + "}\$/"; //这个是正则表达式\w已经等效于[A-Za-z0-9_];
        if (!name.match(new RegExp(reg))) {
            return {
                ok: false,
                message: "只能为" + min + "~" + max + "位数字/字母/下划线！"
            };
        }
        return {
            ok: true,
    ...
    checkUsername: function(name, min, max) {
        var min = min || 8;
        var max = max || 20;

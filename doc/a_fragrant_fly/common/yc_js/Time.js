const Utils = {
    ...
    toTimestamp: (date, num) => {
        ...
        return parseInt(((Date.parse(new Date(date)))+'').substr(0,num));
    },
    ...
    formatDate: (timep, timetype) => {
        var timep = timep || null;
        if (timep && parseInt(timep) != timep) {
            // 判断时间戳是否是数字，不是数字原数据返回
            return timep;
        }
        if (!timep) {
            timep = Date.parse(new Date()) / 1000;
        } else {
            let timp = timep.toString()
            if (timp.length > 10) {
                timep = timp.substring(0, 10);
            }else{
                for (let i = 0; i < (10 - timp.length); i++) {
                    timep = timep * 10
                }
            }
...
var dateUtils = {
    ...
    format: (dateStr, timetype) => {
        ...
        var timetype = timetype || "M-d hh:mm";

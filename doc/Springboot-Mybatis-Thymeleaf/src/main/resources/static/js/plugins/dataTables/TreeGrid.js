...
TreeGrid = function(_config){
	...
	drowHeader = function(){
		...
		for(i=0;i<cols.length;i++){
			var col = cols[i];
			s += "<td align='" + (col.headerAlign || _config.headerAlign || "center");
            if(col.hidden){s +="' hidden='true";}
            s+="' width='" + (col.width || "") + "'>" + (col.headerText || "") + "</td>";
		}
		s += "</tr>";
	}
	...
	drowRowData = function(_rows, _cols, _level, _pid){
		...
		for(var i=0;i<_rows.length;i++){
			...
			s += "<tr id='TR" + id + "' pid='" + ((_pid=="")?"":("TR"+_pid)) + "' openchild='Y' data=\"" + TreeGrid.json2str(row) + "\" rowIndex='" + rownum++ + "'>";
	...
	this.show = function(){
		...
		s = "<table id='" + this.id + "' cellspacing=0 cellpadding=0 width='" + (_config.width || "100%") + "' class='table table-hover'>";
		drowHeader();
		drowData();
		s += "</table>";
	...
	cascadeCheck = function(trId,checked){
		...
		trs.each(function(){
			...
			cascadeCheck($(this).prop("id"),checked);
		});

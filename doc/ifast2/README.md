...
# 写在前面的
1. Fork form: [ifast](https://gitee.com/icron/ifast),
2. 升级框架为SpringbBoot2.1.x， MybatisPlus3.x, thymeleaf3,
3. 修改数据库连接库为PosgreSQL, 同时支持MySQL,

启动项目
1. 导入ifast.postgresql.sql.到postgreSQL
1. 启动项目 mvn spring-boot:run （jdk8, maven3）
1. 访问http://localhost:8088/index
1. 用户名/密码： admin/1
...
# 简介 | Intro
ifast是一款基于Spring Boot 2 + Mybatis + Mybatis Plus 3搭建的快速开发平台。ifast集成了代码生成器，具有高效的开发效率。以Spring Boot为基础框架，Mybatis plus为数据访问层， Apache Shiro为权限授权层，Ehcahe对常用数据进行缓存，基于Bootstrap构建的Admin LTE作为前端框架。

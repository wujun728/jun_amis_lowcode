...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 根据字典加载的下拉框
            showDictSelect: function (datas, name, value) {
                var html = '<select name="' + name + '" class="form-control">';
                $.each(datas, function (index, dict) {
                    var selected = dict.dictValue == ('' + value) ? 'selected="selected"' : '';
                    html += '<option value="' + dict.dictValue + '" ' + selected + '>' + dict.dictLabel + '</option>';
                });
                html += '</select>';
                return html;
            },
            // 根据json加载的下拉框
            showJsonSelect: function (datas, name, value) {
                var html = '<select name="' + name + '" class="form-control"><option value="">请选择...</option>';
                $.each(datas, function (key, val) {
                    var selected = key == ('' + value) ? 'selected="selected"' : '';
        ...
        rySummernote: {
            // 初始化summernote
            init: function (element, placeholder, height) {
                $(element).summernote({
                    placeholder: placeholder,
                    height: height,
                    lang: 'zh-CN',
                    fontNames: [
                        'Arial', 'Arial Black', 'Comic Sans MS', 'Courier New', 'Helvetica Neue', 'Helvetica', 'Impact', 'Lucida Grande',
                        'Tahoma', 'Times New Roman', 'Verdana', '黑体', '微软雅黑', '宋体', '仿宋', '楷体', '隶书', '幼圆'
                    ],
                    followingToolbar: false,
                    callbacks: {
                        onImageUpload: function (files) {
                            $.rySummernote.sendFile(files[0], this);
                        }
                    }
                });
            },
        ...
        tree: {
            ...
            // 获取指定zTree当前被勾选集合
            getTreeCheckedNodes: function (treeId, column) {
        ...
        common: {
            ...
            // ajax请求
            getAjaxData: function (url, params, dataType, funcSuccess, funcError, async, cache) {
                var result;
                $.ajax({
                    async: async || false, //是否异步请求
                    url: url, //请求地址
                    data: params, //参数
                    cache: cache || false, //设置为 false 将不会从浏览器缓存中加载请求信息
                    type: "POST",
                    dataType: dataType,
                    error: (funcError !== null && funcError != undefined) ? funcError : function (XMLHttpRequest, textStatus, errorThrown) {
                        $.modal.alertError("后台服务请求失败！");
                    },
                    success: (funcSuccess !== null && funcSuccess != undefined) ? funcSuccess : function (resultData) {
                        result = resultData;
                    }
                });
                return result;
            ...
            // 取最大整型数字
            maxInt: function (datas) {
                var maxNumber = 0;
                $.each(datas, function (i) {
                    var tmp = parseInt($(this).val());
                    maxNumber = tmp > maxNumber ? tmp : maxNumber;
                });
                return maxNumber;
            }
        }

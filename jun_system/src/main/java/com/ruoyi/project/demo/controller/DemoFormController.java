...
public class DemoFormController {
    ...
    public String localRefresh(ModelMap mmap) {
        ...
        item.set("name", "这条任务数据是由ModelMap传递到页面的，点击添加按钮后会将这条数据替换为新数据");
        item.set("type", "默认");
        item.set("date", "2020.06.10");
        list.add(item);
        mmap.put("tasks", list);
        mmap.put("min", 2);
        mmap.put("max", 10);
    ...
    public String localRefreshTask(String fragment, String taskName, ModelMap mmap) {
        JSONArray list = new JSONArray();
        JSONObject item = new JSONObject();
        item.set("name", StrUtils.defaultIfBlank(taskName, "通过电话销售过程中了解各盛市的设备仪器使用、采购情况及相关重要追踪人"));
        item.set("type", "新增");
        item.set("date", "2018.06.10");
        list.add(item);
        item = new JSONObject();
        item.set("name", "提高自己电话营销技巧，灵活专业地与客户进行电话交流");
        item.set("type", "新增");
        item.set("date", "2018.06.12");
        list.add(item);
        mmap.put("tasks", list);
